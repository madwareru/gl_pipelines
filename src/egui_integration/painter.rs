...
use crate::{Bindings, BlendFactor, BlendState, BlendValue, Buffer, BufferLayout, BufferType, Context, CullFace, Equation, FilterMode, PassAction, Pipeline, PipelineParams, Shader, Texture, TextureFormat, TextureKind, TextureParams, TextureWrap, VertexAttribute, VertexFormat};
        ...
        if let Some([x, y]) = delta.pos {
            ...
            let texture = match &delta.image {
                egui::ImageData::Color(image) => {
                    ...
                    let data: &[u8] = bytemuck::cast_slice(image.pixels.as_ref());
                    Texture::from_data_and_format(ctx, data, params, TextureKind::Texture2D)
                }
                egui::ImageData::Font(image) => {
                    assert_eq!(
                        image.width() * image.height(),
            ...
            match primitive {
                ...
                egui::epaint::Primitive::Callback(callback) => {
                    let info = egui::PaintCallbackInfo {
                        ...
                        screen_size_px: [
                            (screen_size_in_points.0 * dpi.0).round() as _ ,
                            (screen_size_in_points.1 * dpi.1).round() as _,
                        ],
                    };
                    ...
                    if let Some(callback) = callback.callback.downcast_ref::<CallbackFn>() {
                        ...
                        eprintln!(
                            "Warning: Unsupported render callback"
                        );
                    }
                }
            }
    ...
    pub fn paint_job(
        ...
        for mesh in meshes {
            ...
            self.bindings.images[0] = match mesh.texture_id {
                ...
                egui::TextureId::User(id) => unsafe { Texture::from_raw_id(ctx,id as u32) },
            };

            let (width_in_pixels, height_in_pixels) = (
                screen_size_in_points.0 as f32 * pixels_per_point,
                screen_size_in_points.1 as f32 * pixels_per_point
            );
