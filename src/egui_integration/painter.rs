...
use crate::{Bindings, BlendFactor, BlendState, BlendValue, Buffer, BufferLayout, BufferType, Context, Equation, Pipeline, PipelineParams, Shader, TextureKind, VertexAttribute, VertexFormat};
...
impl Painter {
    pub fn new(ctx: &mut Context) -> Painter {
        ...
        let bindings = Bindings {
            ...
            index_buffer,
            images: vec![crate::Texture::empty(ctx)],
        };
        ...
        Painter {
            ...
            egui_texture: crate::Texture::empty(ctx),
        }
    }
    ...
    pub fn paint(
        ...
        let screen_size_in_points = ctx.get_window_size();
        let screen_size_in_points = (
            screen_size_in_points.0 as _,
            screen_size_in_points.1 as _,
        );
    ...
    pub fn paint_job(&mut self, ctx: &mut Context, clip_rect: egui::Rect, mesh: egui::paint::Mesh) {
        let screen_size_in_pixels = ctx.get_window_size();
        let pixels_per_point = ctx.get_dpi();
        ...
        for mesh in meshes {
            ...
            self.bindings.images[0] = match mesh.texture_id {
                egui::TextureId::Egui => self.egui_texture.clone(),
                egui::TextureId::User(id) => unsafe { crate::Texture::from_raw_id(ctx,id as u32) },
            };

            let (width_in_points, height_in_points) = screen_size_in_pixels;

            // From https://github.com/emilk/egui/blob/master/egui_glium/src/painter.rs#L233

            // Transform clip rect to physical pixels:
            let clip_min_x = pixels_per_point.0 * clip_rect.min.x;
            let clip_min_y = pixels_per_point.1 * clip_rect.min.y;
            let clip_max_x = pixels_per_point.0 * clip_rect.max.x;
            let clip_max_y = pixels_per_point.1 * clip_rect.max.y;

            // Make sure clip rect can fit withing an `u32`:
            let clip_min_x = clip_min_x.clamp(0.0, width_in_points as f32 * pixels_per_point.0);
            let clip_min_y = clip_min_y.clamp(0.0, height_in_points as f32 * pixels_per_point.1);
            let clip_max_x = clip_max_x.clamp(clip_min_x, width_in_points as f32 * pixels_per_point.0);
            let clip_max_y = clip_max_y.clamp(clip_min_y, height_in_points as f32 * pixels_per_point.1);
            ...
            ctx.apply_scissor_rect(
                clip_min_x as i32,
                (height_in_points as u32 - clip_max_y) as i32,
